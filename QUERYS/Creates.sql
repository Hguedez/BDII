...
CREATE TABLE Frontera(
    ...
    FK_Lugar number,
    CONSTRAINT FK_Frontera_Lugar FOREIGN KEY (FK_Lugar) references Lugar(Codigo_lugar)
);
...
CREATE TABLE Aerolinea(
    Codigo_aerolinea NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    logo_aerolinea BLOB,
    nombre_aerolinea varchar(100)
);

CREATE TABLE Vuelo(
    Numero_vuelo varchar(15) PRIMARY KEY,
    ...
    constraint FK_aerolinea foreign key(FK_Aerolinea) references Aerolinea(Codigo_aerolinea)
);
...
CREATE TABLE Lugar_Proveedor(
    ...
    Fecha date not null,
    Velocidad_subida varchar(30) not null,
    Velocidad_descarga varchar(30) not null,
...
CREATE TABLE Numero_Infectados(
    codigo_numero_infectados NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    fk_lugar number,
    fecha date,
    cantidad number,
    constraint FK_Lugar_numero_Infectados foreign key(FK_Lugar) references Lugar(Codigo_lugar)
);

CREATE TABLE Fecha_Actual(
    codigo_fecha NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    fecha_actual date
);
...
CREATE TABLE Vuelo_persona(
    Codigo_vuelo_persona NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    FechaInicioFin Fecha_Inicio_Fin,
    FK_Vuelo number,
    FK_Persona number,
    CONSTRAINT FK_Vuelo FOREIGN KEY(FK_Vuelo) references Vuelo(Numero_vuelo),
    CONSTRAINT FK_Vuelo_Persona FOREIGN KEY(FK_Persona) references Persona(Codigo_persona)
);


CREATE TABLE Viaje(
    Codigo_viaje NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    FK_Vuelo_Persona number,
    FK_Lugar number,
    CONSTRAINT FK_Viaje FOREIGN KEY(FK_Vuelo_Persona) references Vuelo_persona(Codigo_vuelo_persona),
    CONSTRAINT FK_Lugar_Viaje FOREIGN KEY(FK_Lugar) references Lugar(Codigo_lugar)
);
